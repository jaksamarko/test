...
import { findTeamMemberByName } from '../data/TeamMembers';
import countapi from 'countapi-js';

const EventLayout = () => {
	const noImageUrl = 'no_image_wide.jpg';
	...
	function fnUpdate(response: { value: number }) {
		console.log(response.value);
		setViewCount(response.value);
	}
	...
	useEffect(() => {
		...
		countapi.visits().then(result => {
			if (result.status === 200) setViewCount(result.value);
		});
		//return setCounterScript(null);
	}, []);

	function getImage() {
		let img: string;
		try {
			img = require(`../assets/images/${postData?.image_url || noImageUrl}`);
		} catch (e) {
			img = require(`../assets/images/${noImageUrl}`);
		}
								...
								<div className="flex flex-wrap items-center p-4 pb-4 sm:p-8">
									<div className="mb-4 mr-5 flex items-center md:mr-10">
										<div className="mr-4 h-10 w-10 overflow-hidden rounded-full">
											<img
												src={findTeamMemberByName(postData?.author!).image}
												alt="image"
												className="w-full"
											/>
										</div>
